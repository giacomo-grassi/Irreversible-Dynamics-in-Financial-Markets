...
def test_joint_regression(rv_beta_path, rv_de_path, hmfsi_beta_path):
    ...
    df = pd.DataFrame(
        {"beta": beta_rv, "De": de_rv, "RV": rv_stress, "HMFSI": hmfsi_stress}
    ).dropna()
    ...
    result = OrderedDict(
        {
            ...
            "coefficients": {
                "const": {
                    "value": round(model.params["const"], 4),
                    "p": round(model.pvalues["const"], 4),
                },
                "De": {
                    "value": round(model.params["De"], 4),
                    "p": round(model.pvalues["De"], 4),
                },
                "RV": {
                    "value": round(model.params["RV"], 4),
                    "p": round(model.pvalues["RV"], 4),
                },
                "HMFSI": {
                    "value": round(model.params["HMFSI"], 4),
                    "p": round(model.pvalues["HMFSI"], 4),
                },
            },
            "interpretation": (
                "RV significant → endogenous channel confirmed. "
                "HMFSI not significant → no direct exogenous channel for β."
                if model.pvalues["HMFSI"] > 0.05
                else "Both RV and HMFSI significant → dual-channel for β."
            ),
        }
    )
    return result
...
def test_correlation_instability(rv_corr_path, hmfsi_corr_path):
    ...
    df = pd.DataFrame(
        {"corr_beta_de": corr_rv, "RV": rv_stress, "HMFSI": hmfsi_stress}
    ).dropna()
...
def test_predictive_regression(hmfsi_corr_path, horizons=None):
    ...
    for h in horizons:
        ...
        model = newey_west_ols(
            df["hmfsi_future"], df[["corr_now", "hmfsi_now"]], max_lags=min(h + 2, 12)
        )

        sig = "***" if model.pvalues["corr_now"] < 0.05 else ""
        print(
            f"  h={h}: ψ={model.params['corr_now']:.4f}, "
...
def test_interaction_model(rv_beta_path, rv_de_path, hmfsi_beta_path):
    ...
    df = pd.DataFrame({"beta": beta_rv, "De": de_rv, "HMFSI": hmfsi_stress}).dropna()
    ...
    result = OrderedDict(
        {
            ...
            "coefficients": {
                ...
                "De_x_HMFSI": {
                    "value": round(model.params["De_x_HMFSI"], 4),
                    "p": round(model.pvalues["De_x_HMFSI"], 4),
                },
            },
            "interpretation": (
                "Significant interaction → HMFSI modifies the β–Dₑ mapping slope."
                if model.pvalues["De_x_HMFSI"] < 0.05
                else "No significant interaction → static mapping is robust to exogenous stress."
            ),
        }
    )
    return result
